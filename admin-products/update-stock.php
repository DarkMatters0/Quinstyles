...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($status === 'out') {
        if ($stocksObj->addStockOut()) {
            echo json_encode(['status' => 'success']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to update stock out.']);
        }
    } elseif ($status === 'in') {
        if ($stocksObj->addStockIn()) {
            echo json_encode(['status' => 'success']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to update stock in.']);
        }
    }
}
?>
